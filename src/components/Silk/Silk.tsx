...
import { forwardRef, useRef, useMemo, useLayoutEffect, useSyncExternalStore, FC, CSSProperties } from 'react';
...
const SilkPlane = forwardRef<Mesh, SilkPlaneProps>(function SilkPlane({ uniforms }, ref) {
  ...
  useFrame((_, delta) => {
    if (ref && 'current' in ref && ref.current) {
      // @ts-expect-error ShaderMaterial uniforms are dynamic in Three.js
...
export const Silk: FC<SilkProps> = ({
  ...
  // SSR hydration safety without cascading re-renders
  ...
  if (!isClient) {
    return (
      <div
        className={className}
        style={{ width: '100%', height: '100%', backgroundColor: '#080808', ...style }}
        aria-hidden="true"
      />
    );
  }
